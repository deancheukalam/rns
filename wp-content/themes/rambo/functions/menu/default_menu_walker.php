...
function rambo_fallback_page_menu( $args = array() ) {
	...
	if ( ! empty($args['show_home']) ) {
		if ( true === $args['show_home'] || '1' === $args['show_home'] || 1 === $args['show_home'] )
			$text = esc_html__('Home','rambo');
		else
			$text = $args['show_home'];
		$class = '';
		if ( is_front_page() && !is_paged() )
			$class = 'class="current_page_item menu-item"';
...
class Rambo_walker_page_menu extends Walker_Page{
	...
	function start_el( &$output, $page, $depth=0, $args = array(), $current_page = 0 ) {
      ...
      if (isset($args['pages_with_children'][$page->ID]) && $depth >= 0) {
		...
		$output .= $indent . '<li class="' . $css_class . '"><a class="dropdown-toggle" href="' . esc_url(get_permalink($page->ID)) . '">' . $link_before . apply_filters( 'the_title', $page->post_title, $page->ID ) . $link_after . '</a>';
