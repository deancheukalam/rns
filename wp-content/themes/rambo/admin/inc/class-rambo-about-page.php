...
if (!class_exists('Rambo_About_Page')) {
	class Rambo_About_Page {
		...
		function __construct() {
			...
			add_action('wp_ajax_rambo_update_rec_acts', array($this, 'update_recommended_actions_watch'));
			add_action('load-themes.php', array($this, 'activation_admin_notice'));
			/* enqueue script and style for welcome screen */
			add_action( 'admin_enqueue_scripts', array( $this, 'style_and_scripts' ) );

			/* load welcome screen */
			add_action( 'rambo_info_screen', array( $this, 'get_started' ), 10 );
			add_action( 'rambo_info_screen', array( $this, 'github_file' ), 40 );
			add_action( 'rambo_info_screen', array( $this, 'welcome_free_pro' ), 50 );
			add_action( 'rambo_info_screen', array( $this, 'recommending_actions' ), 50 );
			add_action( 'rambo_info_screen', array( $this, 'changelog_themes' ), 50 );
			}
	...
	public function style_and_scripts( $hook_suffix ) {
		...
		if ( 'appearance_page_rambo-welcome' == $hook_suffix ) {

			wp_enqueue_style( 'rambo-info-css', get_template_directory_uri() . '/css/bootstrap.css' );

			wp_enqueue_style( 'rambo-responsive-info-css', get_template_directory_uri() . '/css/bootstrap-responsive.css' );

			wp_enqueue_style( 'rambo-info-screen-css', get_template_directory_uri() . '/admin/assets/css/welcome.css' );

			wp_enqueue_style('rambo-theme-info-style', get_template_directory_uri() . '/admin/assets/css/welcome-page-styles.css');

			wp_enqueue_style('rambo_welcome_customizer', get_template_directory_uri() . '/admin/assets/css/welcome_customizer.css');
			...
			wp_enqueue_script('rambo-about-tabs', get_template_directory_uri() . '/admin/assets/js/about-tabs.js', array('jquery'));
		...
		public function welcome_admin_notice() {
			...
			<div class="updated notice is-dismissible">
				<p><?php echo sprintf( esc_html__("Rambo theme is installed. To take full advantage of the features this theme has to offer visit our %1\$s welcome page %2\$s", "rambo"), '<a href="' . esc_url( admin_url( 'themes.php?page=rambo-welcome' ) ) . '">', '</a>' ); ?></p>
				<p><a href="<?php echo esc_url( admin_url( 'themes.php?page=rambo-welcome' ) ); ?>" class="button" style="text-decoration: none;"><?php esc_html_e( 'Get started with Rambo Lite', 'rambo' ); ?></a></p>
			</div>
		<?php
		}
	  ...
	  <div class="container-fluid">
		<div class="row">
		<div class="col-md-12">
			<div class="wrap theme-info-wrap rambo-wrap">
	...
	/**
		...
		public function changelog_themes() {
			...
			?>
			<div id="changelog" class="rambo-tab-pane panel-close">
		...
		public function get_tabs_array() {
			...
			if(count($this->get_useful_plugins()) > 0){
				$tabs_array[]	= 	array(
						'link'      => 'useful_plugins',
						'name'      => esc_html__('Why Upgrade to PRO?', 'rambo'),
			...
			$tabs_array[]	= 	array(
					'link'      => 'free_vs_pro',
					'name'      => esc_html__('Free vs Pro', 'rambo'),
					'file_path' => get_template_directory() . '/admin/tab-pages/free-vs-pro.php',
				);

			$tabs_array[]	= 	array(
					'link'      => 'changelog',
					'name'      => esc_html__('Changelog', 'rambo'),
		...
		public function get_recommended_plugins() {
			...
			$plugins = apply_filters('rambo_recommended_plugins', array());
			return $plugins;
		}

		public function get_useful_plugins() {
			$plugins = apply_filters('rambo_useful_plugins', array());
			return $plugins;
		}
		...
		public function get_plugin_buttion($slug, $name, $function) {
			...
			if (!$is_installed) {
				...
				$button_html        = sprintf('<a class="webriti-plugin-install install-now button-secondary button" data-slug="%1$s" href="%2$s" aria-label="%3$s" data-name="%4$s">%5$s</a>',
					...
					sprintf(esc_html__('Install %s Now', 'rambo'), esc_html($name)),
					esc_html($name),
					esc_html__('Install & Activate', 'rambo')
				);
				...
				$button_html = sprintf('<a class="webriti-plugin-activate activate-now button-primary button" data-slug="%1$s" href="%2$s" aria-label="%3$s" data-name="%4$s">%5$s</a>',
					...
					sprintf(esc_html__('Activate %s Now', 'rambo'), esc_html($name)),
					esc_html($name),
					esc_html__('Activate', 'rambo')
				);
			} elseif ($is_activeted) {
				$button_html = sprintf('<div class="action-link button disabled"><span class="dashicons dashicons-yes"></span> %s</div>', esc_html__('Active', 'rambo'));
				$is_done     = true;
			}
...
function rambo_useful_plugins_array($plugins){
	$plugins[] = array(
					'slug'     => 'elementor',
				);
...
add_filter('rambo_useful_plugins', 'rambo_useful_plugins_array');

function rambo_recommended_plugins_array($plugins){
	$plugins[] = array(
					'name'     => esc_html__('Webriti Companion', 'rambo'),
					'slug'     => 'webriti-companion',
					'function'     => 'webriti_companion_activate',
					'desc'     => esc_html__('It is highly recommended that you install the Webriti Companion plugin to have access to the advance Frontpage sections and other theme features', 'rambo'),
				);
	...
	$plugins[] = array(
					...
					'slug'     => 'contact-form-7',
					'function'     => 'wpcf7',
					'desc'     => esc_html__('it is recommended that you install the Contact Form 7 plugin to show contact form on pages', 'rambo'),
				);
	...
	$plugins[] = array(
					...
					'desc'     => esc_html__('Quickly and Easily display Social Media Icons on your Website.', 'rambo'),
				);
	...
	$plugins[] = array(
                    ...
                    'function' => 'sobw_fs',
                    'desc'     => esc_html__('It is recommended that you install & activate the Seo Optimized Images plugin to dynamically insert SEO Friendly alt attributes and title attributes to your Images.', 'rambo'),
            );
    ...
    $plugins[] = array(
            ...
            'function' => 'sps_fs',
            'desc'     => esc_html__('To display the posts in a beautiful slider with multiple options, install & activate this plugin.', 'rambo'),
    );

	return $plugins;
}
add_filter('rambo_recommended_plugins', 'rambo_recommended_plugins_array');

function Rambo_About_Page() {
	return Rambo_About_Page::get_instance();
}
global $Rambo_About_Page;
$Rambo_About_Page = Rambo_About_Page();
